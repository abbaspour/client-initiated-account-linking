...
describe('onExecutePostLogin', () => {
  ...
  it('should exit unlink if user is does not have link to requested connection', async () => {
    ...
    expect(mockApi.access.deny).toHaveBeenCalledWith(
      `user ${mockEvent.user.user_id} does not have profile against connection: ${mockEvent.request.query.requested_connection}`
    );
  });
  ...
  it('should exit if id_token_hint sub mismatch', async () => {
    ...
    expect(mockApi.access.deny).toHaveBeenCalledWith(
      `sub mismatch. expected ${mockEvent.user.user_id} received auth0|321`
    );
  });
  ...
  it('should redirect to nestedAuthorizeURL with one connection_scope', async () => {
    mockEvent.request.query.requested_connection_scopes =
...
describe('onContinuePostLogin', () => {
  ...
  it('continue should exit if email is not verified', async () => {
    ...
    jwt.verify.mockImplementation((id_token, getKey, signature, cb) => {
      return cb(null, {
        sub: 'auth0|456',
        nonce: 'cb2515ab1456f97027c903f2702f7d06',
  ...
  it('continue should link for valid input', async () => {
    ...
    expect(mockLinkMethod).toHaveBeenCalledWith(
      { id: 'auth0|123' },
      { user_id: 'abc', provider: 'google-oauth2' }
    );
  });
  ...
  it('continue should exit if link throws error', async () => {
    ...
    mockLinkMethod.mockImplementation(() => {
      throw new Error();
    });
  ...
  it('continue should unlink for valid input', async () => {
    ...
    expect(mockUnlinkMethod).toHaveBeenCalledWith({
      id: 'auth0|123',
      user_id: 'abc',
      provider: 'google-oauth2',
    });
  ...
  it('continue should exit if unlink throws error', async () => {
    ...
    mockUnlinkMethod.mockImplementation(() => {
      throw new Error();
    });
