...
exports.onExecutePostLogin = async (event, api) => {
  ...
  if (is_link_request) {
    ...
    const identity_with_req_conn = event.user.identities.filter(
      (i) => i.connection === requested_connection
    );

    if (!identity_with_req_conn || identity_with_req_conn.length === 0) {
      return api.access.deny(
        `user ${event.user.user_id} does not have profile against connection: ${requested_connection}`
      );
    }
  ...
  if (id_token.sub !== event?.user?.user_id) {
    return api.access.deny(
      `sub mismatch. expected ${event?.user?.user_id} received ${id_token.sub}`
    );
  }
...
function splitSubClaim(sub) {
  ...
  return {
    provider: sub.slice(0, firstPipeIndex),
...
async function unlink(event, api, sub) {
  ...
  const unlinkIdentities = event.user.identities.filter(
    (x) => x.connection === connection && x.user_id === user_id_to_unlink
  );
  ...
  try {
    ...
    console.log(
      `successfully unlinked identity ${connection}|${unlink_id} from primary: ${primary_id}`
    );
  } catch (err) {
